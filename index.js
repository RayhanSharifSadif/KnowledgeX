...
app.post("/add_question", function(req, res){
  const course_code = req.body.courseCode;
  res.redirect(`/set_question?course_code=${course_code}`);

})

app.post("/view_answer", function(req, res){
  res.redirect(`/sub_ans`);

})

app.get("/sub_ans", function(req, res) {

  res.render("sub_ans");
});

app.post("/submit_answer", upload.single("answer_file"), function(req, res) {
  const answer_name = req.body.answer_name;
  const file = req.file;
  ...
  const filePath = path.join("upload", file.filename).replace(/\\/g, "/");

  const query = `
    INSERT INTO answers (answer_name, file_path)
    VALUES (?, ?)
  `;

  connection.query(query, [answer_name, filePath], function(err, results) {
    if (err) {
...
app.get("/set_question", function(req, res) {
  ...
  res.render("set_question", {
    course_code: course_code
  });
});

app.post("/submit_question/:course_code", upload.single("question_file"), function(req, res) {
  const course_code = req.params.course_code;
  ...
  const query = `
    INSERT INTO questions (course_code, question_name, file_path)
    VALUES (?, ?, ?)
  `;

  connection.query(query, [course_code, question_name, filePath], function(err, results) {
    if (err) {
...
app.post("/courses", function(req, res) {
  const course_code = req.body.courseCode;
  res.redirect(`/student_view?course_code=${course_code}`);
});
...
app.get("/viewmyquestion", function(req, res) {
  ...
  connection.query(query, function(err, questions) {
    ...
    res.render("vins_question", { questions: questions });
  });
});
...
app.get("/sub_question", function(req, res) {
  const query = "SELECT * FROM questions"; // Fetch all questions without filtering by course code
  ...
  connection.query(query, function(err, questions) {
    ...
    res.render("sub_question", { questions: questions });
  });
});
...
app.get("/view_questions", function(req, res) {
  ...
  connection.query(query, function(err, questions) {
    ...
    res.render("view_questions", { questions: questions });
  });
});

app.post("/submitted_answer", function(req, res) {
  res.redirect(`/subed_ans`);
});

app.get("/subed_ans", function(req, res) {
  const query = "SELECT * FROM answers"; // Fetch all questions without filtering by course code
  ...
  connection.query(query, function(err, answers) {
    ...
    res.render("subed_ans", { answers: answers });
  });
});

app.post("/announcement", function (req, res) {
  const newAnnouncement = req.body.announcement_text;

  const query = `INSERT INTO announcements (announcement_text, date_posted) VALUES (?, NOW())`;

  connection.query(query, [newAnnouncement], function (err, result) {
    if (err) {
...
app.post("/to_announcement", function(req, res) {
  res.redirect(`/announce`);
});
// Route to display announcements
app.get("/announce", function (req, res) {
  const query = `SELECT announcement_text, DATE_FORMAT(date_posted, '%M %d, %Y at %h:%i %p') as formatted_date FROM announcements ORDER BY date_posted DESC`;
  connection.query(query, function (err, results) {
    if (err) {
      console.error("Error retrieving announcements:", err);
      return res.status(500).send("Internal Server Error");
    }
    res.render("announce", { announcements: results });
  });
});
